...
def main():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="nvidia/Cosmos-Predict2-14B-Video2World",
    ...
    parser.add_argument("--frames", type=int, default=129, help="The number of frames in the generated video.")
    parser.add_argument("--fps", type=int, default=25, help="The frames per second of the generated video.")
    ...
    if args.video is not None:
        video = diffusers.utils.load_video(args.video)[-10:]  # use last 10 frames
    else:
        video = None
    ...
    # Connect old and new videos
    if args.video is not None:
        from typing import List
        import PIL.Image
        ori_video: List[PIL.Image.Image] = diffusers.utils.load_video(args.video)
        # compare w*h with args.height*args.width and resize if needed
        if ori_video[0].size != output[0].size:
            ori_video = [img.resize((args.width, args.height), PIL.Image.LANCZOS) for img in ori_video]
        # concatenate last 10 frames of ori_video with first 10 frames of output
        output = ori_video[:-10] + output
        print(f"Connected original video with {len(ori_video)} frames to generated video with {len(output)} frames.")
